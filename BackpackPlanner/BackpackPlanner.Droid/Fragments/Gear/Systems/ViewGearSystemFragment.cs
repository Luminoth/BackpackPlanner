...
namespace EnergonSoftware.BackpackPlanner.Droid.Fragments.Gear.Systems
{
    ...
    {
        private static readonly ILogger Logger = CustomLogger.GetLogger(typeof(ViewGearSystemFragment));
        ...
        protected override int SaveItemResource => Resource.Id.fab_save_gear_system;

        protected override int ResetItemResource => Resource.Id.fab_reset_gear_system;

        protected override int DeleteItemResource => Resource.Id.fab_delete_gear_system;
...
#region Gear Items
        ...
        private void SetGearItemList()
        {
            for(int i=0; i<_gearItems.Length; ++i) {
                GearItem gearItem = _gearItems[i];

                GearItemEntry gearItemEntry = Item.GearItems.FirstOrDefault(x => x.ModelId == gearItem.Id);
                if(null != gearItemEntry) {
                    _selectedGearItems[i] = true;
